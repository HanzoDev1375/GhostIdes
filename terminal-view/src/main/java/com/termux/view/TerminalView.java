...
public final class TerminalView extends View {
  ...
  /** Animated cursor opacity (0..1) used for a smooth cursor blink. */
  private float mCursorAlpha = 1f;

  private ValueAnimator mCursorBlinkAnimator;

  /** Last rendered cursor cell, used to animate a smooth cursor slide while typing. */
  private int mLastRenderedCursorCol;

  private int mLastRenderedCursorRow;
  ...
  /**
   * The current AutoFill type returned for {@link View#getAutofillType()} by {@link
   * #getAutofillType()}.
   *
   * <p>The default is {@link #AUTOFILL_TYPE_NONE} so that AutoFill UI, like toolbar above keyboard
   * is not shown automatically, like on Activity starts/View create. This value should be updated
   * to required value, like {@link #AUTOFILL_TYPE_TEXT} before calling {@link
   * AutofillManager#requestAutofill(View)} so that AutoFill UI shows. The updated value set will
   * automatically be restored to {@link #AUTOFILL_TYPE_NONE} in {@link #autofill(AutofillValue)} so
   * that AutoFill UI isn't shown anymore by calling {@link #resetAutoFill()}.
   */
  @RequiresApi(api = Build.VERSION_CODES.O)
  private int mAutoFillType = AUTOFILL_TYPE_NONE;

  /**
   * The current AutoFill type returned for {@link View#getImportantForAutofill()} by {@link
   * #getImportantForAutofill()}.
   *
   * <p>The default is {@link #IMPORTANT_FOR_AUTOFILL_NO} so that view is not considered important
   * for AutoFill. This value should be updated to required value, like {@link
   * #IMPORTANT_FOR_AUTOFILL_YES} before calling {@link AutofillManager#requestAutofill(View)} so
   * that Android and apps consider the view as important for AutoFill to process the request. The
   * updated value set will automatically be restored to {@link #IMPORTANT_FOR_AUTOFILL_NO} in
  ...
  /**
   * The current AutoFill hints returned for {@link View#getAutofillHints()} ()} by {@link
   * #getAutofillHints()} ()}.
   *
   * <p>The default is an empty `string[]`. This value should be updated to required value. The
   * updated value set will automatically be restored an empty `string[]` in {@link
   * #autofill(AutofillValue)} by calling {@link #resetAutoFill()}.
   */
  ...
  /**
   * This must be called by the hosting activity in {@link Activity#onContextMenuClosed(Menu)} when
   * context menu for the {@link TerminalView} is started by {@link
  ...
  public void inputCodePoint(
      int eventSource, int codePoint, boolean controlDownFromEvent, boolean leftAltDownFromEvent) {
    if (TERMINAL_VIEW_KEY_LOGGING_ENABLED) {
      mClient.logInfo(
          LOG_TAG,
      ...
      case KeyEvent.KEYCODE_PAGE_DOWN:
        ...
        if (shiftDown) {
          ...
          MotionEvent motionEvent =
              MotionEvent.obtain(time, time, MotionEvent.ACTION_DOWN, 0, 0, 0);
          doScroll(
              motionEvent,
              keyCode == KeyEvent.KEYCODE_PAGE_UP ? -mEmulator.mRows : mEmulator.mRows);
          motionEvent.recycle();
          return true;
        }
    }
  ...
  public void updateSize() {
    ...
    if (mEmulator == null || (newColumns != mEmulator.mColumns || newRows != mEmulator.mRows)) {
      mTermSession.updateSize(
          newColumns, newRows, (int) mRenderer.getFontWidth(), mRenderer.getFontLineSpacing());
  ...
  protected void onDraw(Canvas canvas) {
    if (mEmulator == null) {
      ...
      detectCursorMove();
      mRenderer.mCursorAlpha = mCursorAlpha;
  ...
  /**
   * Starts a short slide animation whenever the terminal cursor moves a small distance (e.g. one
   * character forward while typing), giving the cursor a smooth "glide" feel. Large jumps (wrapping
   * to the next line, shell output, screen clears) are skipped.
   */
  private void detectCursorMove() {
    if (mRenderer == null) return;
    int col = mEmulator.getCursorCol();
    int row = mEmulator.getCursorRow();
    int dx = col - mLastRenderedCursorCol;
    int dy = row - mLastRenderedCursorRow;
    mLastRenderedCursorCol = col;
    mLastRenderedCursorRow = row;
    if (dx == 0 && dy == 0) return;
    if (Math.abs(dx) > 3 || Math.abs(dy) > 1) return;

    final float startX = -dx * mRenderer.mFontWidth;
    final float startY = -dy * mRenderer.mFontLineSpacing;
    if (mCursorMoveAnimator != null) mCursorMoveAnimator.cancel();
    mCursorMoveAnimator = ValueAnimator.ofFloat(1f, 0f);
    mCursorMoveAnimator.setDuration(150);
    mCursorMoveAnimator.setInterpolator(new DecelerateInterpolator(1.5f));
    mCursorMoveAnimator.addUpdateListener(
        new ValueAnimator.AnimatorUpdateListener() {
          @Override
          public void onAnimationUpdate(ValueAnimator animation) {
            float progress = (float) animation.getAnimatedValue();
            mRenderer.mCursorOffsetX = startX * progress;
            mRenderer.mCursorOffsetY = startY * progress;
            invalidate();
          }
        });
    mCursorMoveAnimator.addListener(
        new AnimatorListenerAdapter() {
          @Override
          public void onAnimationEnd(Animator animation) {
            mRenderer.mCursorOffsetX = 0f;
            mRenderer.mCursorOffsetY = 0f;
          }
        });
  ...
  private void stopTerminalCursorBlinker() {
    if (mCursorBlinkAnimator != null) mCursorBlinkAnimator.cancel();
    mCursorAlpha = 1f;
    invalidate();
  ...
  private class TerminalCursorBlinkerRunnable implements Runnable {
    ...
    public void run() {
      try {
        if (mEmulator != null) {
          // Keep the cursor logically visible; the animated alpha below drives the
          // blink with a smooth fade instead of a hard on/off toggle.
          mEmulator.setCursorBlinkState(true);

          if (mCursorBlinkAnimator == null) {
            mCursorBlinkAnimator = ValueAnimator.ofFloat(1f, 0f);
            mCursorBlinkAnimator.setRepeatMode(ValueAnimator.REVERSE);
            mCursorBlinkAnimator.setRepeatCount(ValueAnimator.INFINITE);
            mCursorBlinkAnimator.addUpdateListener(
                new ValueAnimator.AnimatorUpdateListener() {
                  @Override
                  public void onAnimationUpdate(ValueAnimator animation) {
            ...
            mCursorBlinkAnimator.addListener(
                new AnimatorListenerAdapter() {
                  @Override
                  public void onAnimationEnd(Animator animation) {
                    mCursorAlpha = 1f;
          ...
          mCursorBlinkAnimator.setDuration(mBlinkRate);
          if (!mCursorBlinkAnimator.isRunning()) mCursorBlinkAnimator.start();
        }
      } finally {
        // Keep the loop alive so the animator is re-started if it ever stops.
